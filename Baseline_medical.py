...
test_loader = DataLoader(dataset=test_data, shuffle=True, batch_size=4, num_workers=1, pin_memory=True)
val_data = []
for i in range(len(val.data)):
   val_data.append([val.data[i], val.clean_target[i]])
val_loader = DataLoader(dataset=val_data, shuffle=True, batch_size=4, num_workers=1, pin_memory=True)
tr_data = []
for i in range(len(tr.data)):
   tr_data.append([tr.data[i], tr.dirty_target[i]])
train_loader = DataLoader(dataset=tr_data, shuffle=True, batch_size=4, num_workers=1, pin_memory=True)
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    print('After ', epoch + 1, 'epoch, Accuracy of test images: %d %%' % (
        100 * correct / total))
    print("Correct: ", correct)
    print("Total: ", total)
