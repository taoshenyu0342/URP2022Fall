...
test_loader = DataLoader(dataset=test_data, shuffle=True, batch_size=8, num_workers=4, pin_memory=True)
val_data = []
for i in range(len(val.data)):
   val_data.append([val.data[i], val.clean_target[i]])
val_loader = DataLoader(dataset=val_data, shuffle=True, batch_size=8, num_workers=4, pin_memory=True)
tr_data = []
for i in range(len(tr.data)):
   tr_data.append([tr.data[i], tr.dirty_target[i]])
train_loader = DataLoader(dataset=tr_data, shuffle=True, batch_size=8, num_workers=4, pin_memory=True)
...
class Net_x(torch.nn.Module):
    def __init__(self, tr):
        ...
        self.x = torch.nn.Parameter(torch.zeros(8).to(device).requires_grad_(True))
...
x = Net_x(tr)
x_opt = torch.optim.Adam(x.parameters(), lr=0.00005)
import torch.optim as optim
criterion = nn.CrossEntropyLoss()
y_opt = optim.SGD(model.parameters(), lr=0.0001, momentum=0.9)
y = model
count = 0 
for x_itr in range(10):
    print("x_itr", x_itr)
    x_opt.zero_grad()
    F_list = []
    with higher.innerloop_ctx(y, y_opt) as (fmodel, diffopt):
        for epoch in range(1):  # loop over the dataset multiple times
            print("epoch", epoch)
            for i, data in enumerate(zip(train_loader,val_loader)):
                print("count", count)
                count = count + 1
                # get the inputs; data is a list of [inputs, labels]
                inputs, labels, inputs2, labels2 = data[0][0],data[0][1],data[1][0],data[1][1]
                inputs, labels, inputs2, labels2 = inputs.to(device),labels.to(device),inputs2.to(device),labels2.to(device)
                ...
                print(loss)
                diffopt.step(loss)
                F_list.append(F.cross_entropy(fmodel(inputs2), labels2.long()).item())
        pmax = F_list.index(max(F_list)) 
        print(pmax)
        out_F = fmodel(inputs2, params=fmodel.parameters(time=pmax + 1))
        loss_F = F.cross_entropy(out_F, labels2.long())
